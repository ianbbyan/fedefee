...
define( 'IDEAPARK_DEMO_URL', 'https://parkofideas.com/goldish/demo/' );
...
if ( ! function_exists( 'ideapark_get_account_link' ) ) {
	function ideapark_get_account_link( $prefix = '' ) {
		...
		return ideapark_wrap( $link_title, '<a class="c-header__button-link c-header__button-link--account" title="' . esc_attr( is_user_logged_in() ? esc_html__( 'Account', 'goldish' ) : esc_html__( 'Login', 'goldish' ) ) . '" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '" rel="nofollow" aria-label="' . esc_attr__( 'My Account', 'goldish' ) . '">', '</a>' );
	}
}
...
if ( ! function_exists( 'ideapark_localize_vars' ) ) {
	function ideapark_localize_vars() {
		...
		$return = [
			...
			'hideButtons'          => ideapark_mod( 'product_grid_layout_mobile' ) != 'compact-mobile' && ideapark_mod( 'hide_add_to_cart_mobile_2_per_row' ) || ideapark_mod( 'product_grid_layout_mobile' ) == 'compact-mobile' && ideapark_mod( 'hide_add_to_cart_mobile_compact' ),
...
if ( ! function_exists( 'ideapark_custom_meta_boxes' ) ) {
	function ideapark_custom_meta_boxes( $meta_boxes ) {
		...
		$fields = [
			[
				'type' => 'custom_html',
				'std'  => sprintf( __( 'If nothing is selected, the %s default settings %s will be used', 'goldish' ), '<a href="' . admin_url( 'customize.php?autofocus[control]=footer_page' ) . '">', '</a>' ),
			],
			[
				'name'    => __( 'HTML block', 'goldish' ),
				'id'      => 'footer_page',
				'type'    => 'select',
				'std'     => 'default',
				'options' => $options,
	...
	function ideapark_breadcrumb_list() {
		...
		if ( is_front_page() ) {
			...
			$array_list[] = [
				...
				'title'    => __( 'Tag', 'goldish' ) . ': ' . $current_tag_name,
				'location' => 'is_tag',
			];
		} else if ( is_author() ) {
...
if ( ! function_exists( 'ideapark_social_networks' ) ) {
	function ideapark_social_networks( $filter = [] ) {
		$networks = [
			'facebook'  => __( 'Facebook', 'goldish' ),
			'instagram' => __( 'Instagram', 'goldish' ),
			'vk'        => __( 'VK', 'goldish' ),
			'ok'        => __( 'OK', 'goldish' ),
			'telegram'  => __( 'Telegram', 'goldish' ),
			'whatsapp'  => __( 'Whatsapp', 'goldish' ),
			'line'      => __( 'Line', 'goldish' ),
			'wechat'    => __( 'WeChat', 'goldish' ),
			'viber'     => __( 'Viber', 'goldish' ),
			'discord'   => __( 'Discord', 'goldish' ),
			'twitter'   => __( 'X (Twitter)', 'goldish' ),
			'youtube'   => __( 'YouTube', 'goldish' ),
			'vimeo'     => __( 'Vimeo', 'goldish' ),
			'snapchat'  => __( 'Snapchat', 'goldish' ),
			'tiktok'    => __( 'TikTok', 'goldish' ),
			'linkedin'  => __( 'LinkedIn', 'goldish' ),
			'flickr'    => __( 'Flickr', 'goldish' ),
			'pinterest' => __( 'Pinterest', 'goldish' ),
			'tumblr'    => __( 'Tumblr', 'goldish' ),
			'github'    => __( 'Github', 'goldish' ),
		];
		if ( $filter ) {
...
if ( IDEAPARK_IS_AJAX_SEARCH ) {
	...
	add_filter( 'elementor/fonts/groups', function ( $font_groups ) {
		if ( get_theme_mod( 'custom_fonts' ) ) {
			$font_groups = array_merge( [ 'ip-custom-fonts' => __( 'Custom', 'goldish' ) ], $font_groups );
		}
