(function ($, root, undefined) {
	...
	$(function () {
		...
		$(document.body)
			...
			.on('click', ".js-product-modal", function (e) {
				...
				var variation_id = $product.find('.c-product__atc-wrap .variation_id').val();
				ideapark_require([
			...
			.on('click', '.c-product-grid__thumb-wrap a', function (e) {
				if (ideapark_is_mobile_layout && !ideapark_wp_vars.hideButtons) {
					...
					if ($item.hasClass('c-product-grid__item--compact-mobile') && (ideapark_wp_vars.withButtons || $atc.length)) {
						e.preventDefault();
					}
				}
			})
	...
	root.ideapark_set_notice_offset = function (offset) {
		...
		if ($notice.length) {
			if (typeof offset !== 'number') {
				...
				if ((ideapark_sticky_mobile_active || !$ideapark_store_notice_top.length && !$ideapark_advert_bar_above.length) && $window.width() < 768) {
					offset += $ideapark_mobile_sticky_row.outerHeight();
				}
			}
			$notice.css({
	...
	root.ideapark_menu_fix_position = function ($ul) {
		if (!ideapark_is_mobile_layout) {
			...
			var container_width = is_fullwidth ? window_width + (is_HTML_block ? 0 : (item_space * 2)) : 1170;
			var container_left = is_fullwidth ? (is_HTML_block ? 0 : -item_space) : $('.js-simple-container').offset().left;
				...
				if (!$this.hasClass('owl-carousel')) {
					$this
						...
						.owlCarousel({
							...
							responsive   : {
								0   : {
									nav: $this.hasClass('h-carousel--nav-mobile'),
								},
								1190: {
									nav: true,
								},
