...
class Ideapark_Importer {
	...
	function importer_page() {
		...
		if ( ! empty( $themes ) || IDEAPARK_DEMO ) {
			...
			if ( ( $code = ideapark_get_purchase_code() ) && $code !== IDEAPARK_SKIP_REGISTER ) {
				...
				$output .= '<p class="submit">' . ( PHP_OS == "Darwin" || ! IDEAPARK_DEMO ? '<button class="button button-primary" ' . ( ! empty( $this->importer['steps'] ) ? ' style="display:none" ' : '' ) . ' id="ip-import-submit">' . __( 'Import', 'ideapark-goldish' ) . '</button> ' : '' ) . '</p>';
				$output .= '<div class="ip-loading-progress' . ( ! empty( $this->importer['steps'] ) ? ' paused' : '' ) . '"><div class="ip-loading-bar"><div class="ip-loading-state"></div><div class="ip-loading-info">' . __( 'Progress', 'ideapark-goldish' ) . ': 0%...</div></div><div class="ip-import-output">' . __( 'Prepare data...', 'ideapark-goldish' ) . '</div></div>';
				if ( ! empty( $this->importer['steps'] ) ) {
					$percent = round( ( $this->importer['base']->step_done / $this->importer['base']->step_total ) * 100 );
					$output  .= '<div class="ip-import-continue">' . ( PHP_OS == "Darwin" || ! IDEAPARK_DEMO ? '<button type="button" class="button button-primary" id="ip-import-continue" data-percent="' . esc_attr( $percent ) . '">' . __( 'Continue', 'ideapark-goldish' ) . '</button>&nbsp;&nbsp;&nbsp;<button type="button" class="button button-default" id="ip-import-cancel" data-percent="' . esc_attr( $percent ) . '">' . __( 'Cancel', 'ideapark-goldish' ) . '</button> ' : '' ) . '</div>';
				}
			...
			if ( defined( 'IDEAPARK_DEMO' ) && IDEAPARK_DEMO ) {
				$output .= '<p><a href="" style="color: #F0F0F1;" onclick="return false;" id="ip-export-submit">' . __( 'Export', 'ideapark-goldish' ) . '</a></p>';
			}
		} else {
	...
	function importer() {
		...
		if ( isset( $_REQUEST['stage'] ) && $_REQUEST['stage'] == 'cancel' ) {
			...
			if ( ! empty( $_REQUEST['import_option'] ) && ! empty( $_REQUEST['import_demo'] ) ) {
				...
				if ( ! ideapark_is_dir( $theme_title_fn = $this->importer_dir . '/' . $this->demo_content_folder . '/' ) ) {
					$this->import_response( 'error', __( 'Error: Can`t open file: ', 'ideapark-goldish' ) . $theme_title_fn );
				}
		...
		if ( empty( $this->importer['steps'][0] ) ) {
			...
			if ( empty( $this->importer['steps'][0] ) ) {
				$this->import_response( 'error', __( 'Error: Select the data you want to import', 'ideapark-goldish' ) . ( $this->is_file_transient ? ' (file)' : ' (db)' ) );
			}
		}

		if ( empty( $this->importer['import_demo_content_folder'] ) ) {
			$this->import_response( 'error', __( 'Error: Selected data is empty', 'ideapark-goldish' ) . ( $this->is_file_transient ? ' (file)' : ' (db)' ) );
		}
					...
					if ( $step == 'prepare' ) {
						...
						if ( ! $_result || ! ideapark_is_file( $this->session_fn_start ) ) {
							if ( get_filesystem_method() !== 'direct' ) {
								$this->import_response( 'error', __( 'Error: No access to data folder. Try to add line "define(\'FS_METHOD\',\'direct\');" to the file wp-config.php', 'ideapark-goldish' ) );
							} else {
								$this->import_response( 'error', __( 'Error: Uploads folder must be writable - ', 'ideapark-goldish' ) . IDEAPARK_UPLOAD_DIR );
							}
						}
					}
				} catch ( Exception $e ) {
...
//				] );
	...
	function export_content() {
		...
		$wpdb->query( "DELETE FROM $wpdb->postmeta WHERE meta_key IN ('_elementor_data_goldish', '_ideapark_variation_images')" );
