 ...
 * Version: 3.37
 * Description: Core plugin for Goldish theme.
...
define( 'IDEAPARK_GOLDISH_FUNC_VERSION', '3.37' );
...
if ( empty( $theme_obj ) || strtolower( $theme_obj->get( 'TextDomain' ) ) != 'goldish' && strtolower( $theme_obj->get( 'TextDomain' ) ) != 'goldish-child' ) {
	...
	add_filter( 'plugin_row_meta', function ( $links, $file ) {
		if ( $file == plugin_basename( __FILE__ ) ) {
			$row_meta = [
				'warning' => '<b style="vertical-align:middle;display:inline-flex;align-items:center;border:solid 2px #dc3545;padding: 2px 10px;color: #dc3545"><span class="dashicons dashicons-warning" style="margin-right: 5px;"></span>' . esc_html__( 'The Goldish theme is not activated! This plugin works only with Goldish theme', 'ideapark-goldish' ) . '</b>',
			];
...
if ( ! empty( $theme_obj ) && version_compare( IDEAPARK_GOLDISH_FUNC_VERSION, $theme_obj->parent() ? $theme_obj->parent()->get( 'Version' ) : $theme_obj->get( 'Version' ), '!=' ) ) {

	add_filter( 'plugin_row_meta', function ( $links, $file ) {
		if ( $file == plugin_basename( __FILE__ ) ) {
			$row_meta = [
				'warning' => '<b style="vertical-align:middle;display:inline-flex;align-items:center;border:solid 2px #dc3545;padding: 2px 10px;color: #dc3545;"><span class="dashicons dashicons-warning" style="margin-right: 5px;"></span>' . sprintf( esc_html__( 'The Goldish theme version and the theme core plugin version must be the same. Please update the plugin to version %s', 'ideapark-goldish' ), IDEAPARK_VERSION ) . '</b>',
			];
		...
		$meta_boxes[] = [
			...
			'fields'     => [
				[
					...
					'desc' => __( 'Enter the url to product video (Youtube, Vimeo, mp4, m4v, webm, ogv, flv etc.).', 'ideapark-goldish' ),
				...
				[
					'name' => '',
					'desc' => __( 'Video (mp4, m4v, webm, ogv, flv) playback in the grid.', 'ideapark-goldish' ),
...
//			'context'    => 'side',
		...
		$meta_boxes[] = [
			...
			'title'  => __( 'Feature', 'ideapark-goldish' ),
			'panel'  => '',
			'fields' => [
				[
					'id'         => 'product_feature',
					...
					'fields'     => [
						...
						[
							'name' => __( 'Description', 'ideapark-goldish' ),
							'id'   => 'description',
							'type' => 'text',
						],
					],
	...
	function ideapark_admin_enqueue_scripts( $hook ) {
		if (
			...
			if ( is_object( $screen ) && $screen->post_type == 'post' ) {
				...
				wp_localize_script( 'ideapark-goldish-post-format', 'ideapark_post_format_vars', [
...
if ( ! function_exists( 'ideapark_is_network_activated' ) ) {
	function ideapark_is_network_activated() {
		...
		return is_multisite() && is_plugin_active_for_network( 'ideapark-goldish/ideapark-goldish.php' );
	}
}
...
if ( ! function_exists( 'ideapark_theme_notice' ) ) {
	function ideapark_theme_notice() {
		...
		$message          = __( 'You have not registered the theme yet! Please <a href="%1$s">enter your purchase code</a> or <a href="%2$s" target="_blank">get a new license here</a>.', 'ideapark-goldish' );
...
add_shortcode( 'ip-wishlist-share', function ( $atts ) {
	if ( function_exists( 'ideapark_wishlist' ) && ( $ip_wishlist_ids = ideapark_wishlist()->ids() ) && ideapark_mod( 'wishlist_share' ) && ! ideapark_wishlist()->view_mode() ) {
		...
		$share_links    = [
			'facebook'  => '<a class="c-post-share__link" target="_blank" href="//www.facebook.com/sharer.php?u=' . $share_link_url . '" title="' . esc_html__( 'Share on Facebook', 'ideapark-goldish' ) . '"><i class="ip-facebook c-post-share__icon c-post-share__icon--facebook"></i></a>',
			'twitter'   => '<a class="c-post-share__link" target="_blank" href="//twitter.com/share?url=' . $share_link_url . '" title="' . esc_html__( 'Share on Twitter', 'ideapark-goldish' ) . '"><i class="ip-twitter c-post-share__icon c-post-share__icon--twitter"></i></a>',
			'pinterest' => '<a class="c-post-share__link" target="_blank" href="//pinterest.com/pin/create/button/?url=' . $share_link_url . '&amp;description=' . urlencode( get_the_title() ) . '" title="' . esc_html__( 'Pin on Pinterest', 'ideapark-goldish' ) . '"><i class="ip-pinterest c-post-share__icon c-post-share__icon--pinterest"></i></a>',
			'linkedin'  => '<a class="c-post-share__link" target="_blank" href="//www.linkedin.com/sharing/share-offsite/?url=' . $share_link_url . '" title="' . esc_html__( 'Share on LinkedIn', 'ideapark-goldish' ) . '"><i class="ip-linkedin c-post-share__icon c-post-share__icon--linkedin"></i></a>',
			'whatsapp'  => '<a class="c-post-share__link" target="_blank" href="//wa.me/?text=' . $share_link_url . '" title="' . esc_html__( 'Share on Whatsapp', 'ideapark-goldish' ) . '"><i class="ip-whatsapp c-post-share__icon c-post-share__icon--whatsapp"></i></a>',
			'telegram'  => '<a class="c-post-share__link" target="_blank" href="//telegram.me/share/url?url=' . $share_link_url . '" title="' . esc_html__( 'Share on Telegram', 'ideapark-goldish' ) . '"><i class="ip-telegram c-post-share__icon c-post-share__icon--telegram"></i></a>',
		]; ?>
		...
		<div class="c-wishlist__share">
			...
			<?php if ( $content ) { ?>
				...
				<span
					class="c-wishlist__share-title"><?php esc_html_e( 'Share Wishlist:', 'ideapark-goldish' ); ?></span>
...
add_shortcode( 'ip-post-share', function ( $atts ) {
	...
	$share_links = [
		'facebook'  => '<a class="c-post-share__link" target="_blank" href="//www.facebook.com/sharer.php?u=' . $esc_permalink . '" title="' . esc_html__( 'Share on Facebook', 'ideapark-goldish' ) . '"><i class="ip-facebook c-post-share__icon c-post-share__icon--facebook"></i></a>',
		'twitter'   => '<a class="c-post-share__link" target="_blank" href="//twitter.com/share?url=' . $esc_permalink . '" title="' . esc_html__( 'Share on Twitter', 'ideapark-goldish' ) . '"><i class="ip-twitter c-post-share__icon c-post-share__icon--twitter"></i></a>',
		'pinterest' => '<a class="c-post-share__link" target="_blank" href="//pinterest.com/pin/create/button/?url=' . $esc_permalink . ( $product_image ? '&amp;media=' . esc_url( $product_image[0] ) : '' ) . '&amp;description=' . urlencode( get_the_title() ) . '" title="' . esc_html__( 'Pin on Pinterest', 'ideapark-goldish' ) . '"><i class="ip-pinterest c-post-share__icon c-post-share__icon--pinterest"></i></a>',
		'linkedin'  => '<a class="c-post-share__link" target="_blank" href="//www.linkedin.com/sharing/share-offsite/?url=' . $esc_permalink . '" title="' . esc_html__( 'Share on LinkedIn', 'ideapark-goldish' ) . '"><i class="ip-linkedin c-post-share__icon c-post-share__icon--linkedin"></i></a>',
		'whatsapp'  => '<a class="c-post-share__link" target="_blank" href="//wa.me/?text=' . $esc_permalink . '" title="' . esc_html__( 'Share on Whatsapp', 'ideapark-goldish' ) . '"><i class="ip-whatsapp c-post-share__icon c-post-share__icon--whatsapp"></i></a>',
		'telegram'  => '<a class="c-post-share__link" target="_blank" href="//telegram.me/share/url?url=' . $esc_permalink . '" title="' . esc_html__( 'Share on Telegram', 'ideapark-goldish' ) . '"><i class="ip-telegram c-post-share__icon c-post-share__icon--telegram"></i></a>',
	];
