...
class Ideapark_Variation_Gallery {
	...
	public function admin_enqueue_scripts() {
		...
		wp_localize_script( 'ideapark-variation-gallery-admin', 'ideapark_variation_vars', [
			...
			'main_image'   => esc_html__( 'First you need to select the variation image above', 'ideapark-goldish' ),
		] );
	}
	...
	public function default_import_column_name( $columns ) {
		$columns[ esc_html__( 'Variation Gallery', 'ideapark-goldish' ) ] = $this->column_id;
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				throw new Exception( sprintf( __( 'Not able to attach "%s".', 'ideapark-goldish' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			throw new Exception( sprintf( __( 'Unable to use image "%s".', 'ideapark-goldish' ), $url ), 400 );
		}
