	...
	public function admin_enqueue_scripts( $hook = '' ) {
		...
		if ( $is_sortable_tax || $is_sortable_post_type ) {
			...
			wp_localize_script( $this->_token . '-sort', 'ideapark_sort_vars', [
				'notice'          => ( ( ! empty( $ideapark_goldish_sort_notice ) && ( $index = $is_sortable_tax ? $_GET['taxonomy'] : $_GET['post_type'] ) && array_key_exists( $index, $ideapark_goldish_sort_notice ) ) ) ? esc_html( $ideapark_goldish_sort_notice[ $index ] ) : '',
				'disableSortable' => esc_html__( 'Disable sortable', 'ideapark-goldish' )
			] );
		}
