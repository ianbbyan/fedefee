...
class Wpos_Anylc_Admin {
	...
	public $curr_version		= POPUPAOC_VERSION;
	...
	function __construct() {
		...
		if (!empty($wpos_analytics_module) && isset($wpos_analytics_module[POPUPAOC_PLUGIN_BASENAME])) {
			$this->plugin_id = $wpos_analytics_module[POPUPAOC_PLUGIN_BASENAME]['id'];
		}
	...
	public function wpos_monthly_cron_hook_fn() {
		...
		if ( !empty( $wpos_analytics_module ) && isset( $wpos_analytics_module[POPUPAOC_PLUGIN_BASENAME]) ) {
			$plugin_data = $wpos_analytics_module[POPUPAOC_PLUGIN_BASENAME];
	...
	public function wpos_process_monthly_data() {
		...
		$data = wpos_anylc_optin_data(POPUPAOC_SLUG, '', true);
	...
	public function wpos_rest_api_init() {
		register_rest_route( POPUPAOC_SLUG . '/v1', '/analytics/', [
			'methods'  => 'POST',
